<script lang="ts">
  ...
  async function openGithub() {
    try { await openRepository(); }
    catch (reason) { status = t("settings.openGithubFailed", { error: localizedError(reason) }); }
  }

  async function openHomepage() {
    try { await openWebsite(); }
    catch (reason) { status = t("settings.openWebsiteFailed", { error: localizedError(reason) }); }
  }
    ...
    <Tabs.Content value={panelTab} class={`settings-content${panelTab === "updates" ? " updates-content" : ""}`} tabindex={-1} onkeydown={onContentKeydown}>
      {#if settings}
        ...
        {:else}
          ...
          <div class="about"><img src="/app-icon-rounded.png" alt={t("settings.iconAlt")} /><h2>ClipClop</h2><p>{t("settings.tagline")}</p><small>{t("settings.version", { version: displayVersion(appVersion) })}</small><div class="about-links"><button class="website" onclick={() => void openHomepage()}><span class="brand-mark" aria-hidden="true"></span>clipclop.io</button><button class="github" aria-label={t("settings.github")} title={t("settings.github")} onclick={() => void openGithub()}><Icon icon={githubIcon} width={18} aria-hidden="true" /></button></div>
  ...
  <AlertDialog.Root open={confirmDemo} onOpenChange={(open) => confirmDemo = open}>
    <AlertDialog.Portal>
      ...
      <AlertDialog.Content class="demo-dialog" onOpenAutoFocus={(event) => { event.preventDefault(); cancelDemoButton?.focus(); }}>
        <div class="dialog-copy"><AlertDialog.Title>{t("settings.demoConfirmTitle")}</AlertDialog.Title><AlertDialog.Description>{t("settings.demoConfirmHelp")}</AlertDialog.Description><p class="dialog-note">{t("settings.demoExternalNote")}</p></div>
