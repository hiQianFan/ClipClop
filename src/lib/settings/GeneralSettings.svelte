<script lang="ts">
  ...
  import { getAutoPastePermissionStatus, type AutoPastePermissionStatus } from "$lib/onboarding/api";
  import { localizedError, t } from "$lib/i18n/index.svelte";
  import { openFilePreviewSettings, openQuicklookInstallPage, type Settings, type TrayClickAction } from "./api";
  import type { ShortcutPlatform } from "./shortcuts";

  let { settings = $bindable(), platform, onquickstart, onautopaste, onerror, heading = $bindable() }: {
    settings: Settings;
  ...
  let previewChecking = $state(true);
  let autoPastePermission = $state<AutoPastePermissionStatus | null>(null);
  ...
  onMount(() => {
    ...
    if (platform === "macos") void refreshAutoPastePermission();
  });

  async function refreshAutoPastePermission() {
    try { autoPastePermission = await getAutoPastePermissionStatus(); }
  ...
  function windowsPreviewHelp() {
    if (previewChecking) return t("settings.quicklookCheckingHelp");
    if (previewCapability.provider === "quicklook") return t("settings.quicklookReady");
    if (previewCapability.reason === "unsupported_install") return t("settings.quicklookUnsupported");
    if (previewCapability.reason === "elevated") return t("settings.peekElevated");
    if (previewCapability.reason === "not_installed") return t("settings.quicklookNotInstalled");
    return t("settings.quicklookUnavailable");
  }
...
{#if platform === "macos"}<h2>{t("settings.permissions")}</h2><div class="row"><span><strong>{t("settings.autoPaste")}</strong><small>{t("settings.autoPasteHelp")}</small><small aria-live="polite">{autoPastePermission?.status === "ready" ? t("onboarding.permission.ready") : autoPastePermission ? t("onboarding.permission.required") : t("onboarding.permission.checking")}</small></span><button onclick={onautopaste}>{t("settings.manage")}</button></div>{/if}
{#if platform === "macos"}<div class="row"><span><strong>{t("settings.filePreview")}</strong><small>{t("settings.filePreviewHelp")}</small><small>{t("settings.filePreviewVerify")}</small></span><button onclick={() => void run(openFilePreviewSettings)}>{t("settings.grant")}</button></div>{/if}
{#if platform === "windows"}
  <div class="row">
    <span>
      <span class="preview-heading">
        <strong>{t("settings.systemPreview")}</strong>
        {#if !previewChecking && previewCapability.version}
          <small class="preview-meta">{t("settings.quicklookVersion", { version: previewCapability.version })}</small>
        {/if}
      </span>
      <small>{windowsPreviewHelp()}</small>
    </span>
    <div class="preview-actions" aria-live="polite">
      {#if previewChecking}
        <small class="preview-status">{t("settings.quicklookChecking")}</small>
      {:else}
        {#if previewCapability.reason === "not_installed"}
          <button onclick={() => void run(openQuicklookInstallPage)}>{t("settings.quicklookInstall")}</button>
        {:else if previewCapability.reason === "unsupported_install"}
          <button onclick={() => void run(openQuicklookInstallPage)}>{t("settings.quicklookInstallDesktop")}</button>
        ...
        {:else if previewCapability.provider === "unavailable"}
