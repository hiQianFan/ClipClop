<script lang="ts">
  ...
  import { openFilePreviewSettings, openPowertoysInstallPage, type Settings, type TrayClickAction } from "./api";
...
{#if platform === "windows"}<div class="row"><span><strong>{t("settings.filePreview")}</strong><small>{windowsPreviewHelp()}</small></span>{#if previewCapability.reason === "not_installed"}<button onclick={() => void run(openPowertoysInstallPage)}>{t("settings.peekInstall")}</button>{/if}</div>{/if}
