...
<div class="row">
  ...
  <div class="actions">
    ...
    <button bind:this={button} class:ready={!configuring && !failed && status === "ready"} disabled={restarting} onclick={() => configuring ? void restart() : failed ? void refresh() : void configure("accessibility")}>
...
<style>
  .row{min-height:68px;padding-block:12px;display:flex;align-items:center;justify-content:space-between;gap:24px;border-bottom:1px solid var(--hairline)}.row>span{flex:1;display:flex;flex-direction:column;gap:3px}strong{font-size:var(--fs-body)}small,p{color:var(--text-3);font-size:var(--fs-ui);line-height:1.5}.actions{display:flex;align-items:center;gap:8px}button,.actions :global(.refresh){min-height:32px;padding:0 12px;border:0;border-radius:var(--radius-md);color:var(--text-2);background:transparent;font-size:var(--fs-ui);white-space:nowrap}button:hover,.actions :global(.refresh:hover){color:var(--text-1);background:var(--bg-hover)}button:focus-visible,.actions :global(.refresh:focus-visible){outline:2px solid var(--text-1);outline-offset:2px}button:disabled,.actions :global(.refresh:disabled){opacity:.45}.actions :global(.refresh){display:flex;align-items:center;justify-content:center;width:32px;padding:0}button.ready{color:var(--success);background:color-mix(in srgb,var(--success) 14%,transparent);font-weight:600}button.ready:hover{color:var(--success);background:color-mix(in srgb,var(--success) 22%,transparent)}.tooltip{padding:6px 10px;background:var(--bg-raised);color:var(--text-1);border:1px solid var(--hairline);border-radius:var(--radius-md);font-size:var(--fs-ui)}
</style>
