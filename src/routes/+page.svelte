<script lang="ts">
  ...
  import { clearHistory, copyClip, deleteClip, getClip, listClips } from "$lib/clips/api";
  ...
  async function select(id: string | null) {
    ...
    try {
      ...
      if (version === requestVersion) detail = next;
    } catch (reason) {
    ...
    {#if detail}
      <div class="preview-body">
        ...
        {:else if detail.content_type === "image"}
          <div class="image-placeholder">图片 · {String(detail.metadata.width ?? "?")}×{String(detail.metadata.height ?? "?")}</div>
        {:else}
