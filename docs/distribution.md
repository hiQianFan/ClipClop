...
- `bundle.yml` 可手动运行（必须输入与应用一致的版本），或在推送语义化 `vX.Y.Z` tag 时构建 Universal DMG 和 Windows x64 NSIS，生成 updater artifacts/`latest.json` 并创建 Draft GitHub Release。打包前会校验触发版本、`package.json`、Cargo package 与 Tauri 配置版本一致，并重新执行前端测试/检查/构建和 Rust fmt/Clippy/test；两个平台串行更新共享的 `latest.json`，避免并发覆盖。
