...
- 纯文本、链接、颜色、代码、图片和文件引用的数据模型与持久化；v1 不保存 HTML/RTF。
- 剪贴板轮询监听、去重与保留期清理；不按 concealed/transient 或内容语义自动过滤。
- 单一复制写回；v1 不实现 rich/plain 双模式或 direct paste。
...
- 设置页面：固定快捷键说明、开机启动、保留期、主题和 ignored apps。
